...
__Vectors
	...
	AREA lab_1_q2_reverse_array_in_memory, CODE, READONLY
	ENTRY
...
loop_non_overlapping_shift
	...
	; address of last value in 2nd array becomes address of 2nd array, R4 is left as is

	LDR R1, =DST_ARR      ; temporarily copy first data segment array to R4

	MOV R3, #0            ; reset counter to 0

loop_shift_back_while_reversing
	LDR R2, [R4, #-4]!    ; decrement second array's address permanently and store value in tmp R2 and  in data segment

    STR R2, [R1], #4      ; store value in first array in data segment and increment it's address

	ADD R3, #0x1          ; R3 is i; increment i
	TEQ R3, #0xA    	  ; if i == 10, XOR is 0, Z = 1, exit loop
	BNE loop_shift_back_while_reversing			  ; else if Z == 0 goto loop
